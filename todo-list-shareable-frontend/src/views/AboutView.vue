<template>
  ...
  <div class="relative overflow-x-auto shadow-md sm:rounded-lg my-3 border-t-2 border-blue-900">
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
      ...
      <tbody>
      <tr v-for="(accounts, index) in accounts" :key="index" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
        <th scope="row" class="px-6 py-4 text-gray-900 dark:text-white whitespace-nowrap">
          {{accounts.username}}
        </th>
<!--        <th scope="row" class="px-6 py-4 text-gray-900 dark:text-white whitespace-nowrap">-->
<!--          <select @change="onChange($event,accounts.id)" class="form-select appearance-none block w-50 px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding bg-no-repeat border border-solid border-gray-300-->
...
export default defineComponent({
  ...
  methods:{
    async getAccounts() {
      try {
        await axios.get( "http://161.97.99.214:3000/accounts/").then(response => {
