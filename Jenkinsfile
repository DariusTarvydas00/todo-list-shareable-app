pipeline {
    ...
    stages{
        ...
        stage("Deliver to registry") {
            steps{
            sh 'docker-compose -f grafana/docker-compose-graf.yaml up -d influxdb grafana'
            sh 'docker-compose -f grafana/docker-compose-graf.yaml run k6 run grafana/scripts/ewoks.js'
